...
```bash
docker pull acactown/gitlab-uppoints:latest
```
...
```bash
docker pull acactown/gitlab-uppoints:v2
```
...
docker run --name=gitlab -d \
  -v /opt/gitlab/data:/home/git/data \
  -v /opt/gitlab/mysql:/var/lib/mysql acactown/gitlab-uppoints:v2
```
...
docker run --name=gitlab -it --rm \
  ...
  -v /opt/gitlab/data:/home/git/data \
  acactown/gitlab-uppoints:v2 app:rake gitlab:setup
```
...
docker run --name=gitlab -it --rm [OPTIONS] \
  acactown/gitlab-uppoints:v2 app:rake gitlab:backup:create
```
...
```bash
docker run --name=gitlab -it --rm [OPTIONS] \
  acactown/gitlab-uppoints:v2 app:rake gitlab:backup:restore
```
...
```bash
docker run --name=gitlab -d [OPTIONS] acactown/gitlab-uppoints:v2
```
...
docker run --name=gitlab -d [OPTIONS] \
  acactown/gitlab-uppoints:v2 app:rake gitlab:env:info
```
...
```bash
docker run --name=gitlab -d [OPTIONS] \
  acactown/gitlab-uppoints:v2 app:rake gitlab:import:repos
```
